...
public sealed partial class MainPage : Page, INotifyPropertyChanged
{
    ...
    private static readonly string EMPTY_SEQUENCE_GRAPH_SYMBOL = "λ";

    private string[] _rules = new[] { "" };
    private string[] _alphabet = new[] { "" };
    private string[] _stack_alphabet = new[] { "" };
    private string[] _translate_alphabet = new[] { "" };
    ...
    public MainPage()
    {
        ...
        handlers = new(){
            null,
            RuleChangingAsync,
            SequenceChangingAsync,
            StackChangingAsync,
            RuleChangingAsync,
            StackMultipleChangingAsync,
            TranslateSequenceChangingAsync
        };
    }
    ...
    private readonly List<string> headerLabels = new() {
                "Номер",
                "Текущее состояние",
                "Символ цепочки",
                "Верхний символ стека",
                "Следующее состояние",
                "Стек",
                "Символ перевода"
            };

    private readonly List<TypedEventHandler<TextBox, TextBoxTextChangingEventArgs>?> handlers = new();
    ...
    private void ParseGrammar()
    {
        ...
        _translate_alphabet = ParseGrammarSetPart(grm[3]).Distinct().ToArray();
        _start_rule = grm[4];
        _start_stack_symbol = grm[5];
        _end_rules = ParseGrammarSetPart(grm[6]).Distinct().ToArray();
    ...
    private async void TranslateSequenceChangingAsync(object sender, TextBoxTextChangingEventArgs e)
    {
        async Task<bool> UserKeepsTyping()
        {
            var txt = ((TextBox)sender).Text;
        ...
        foreach (var symbol in textBox.Text)
        {
            if (!_translate_alphabet.Contains(symbol.ToString()))
            {
                textBox.Text = "";
                break;
            }
    ...
    public void CreateGraph()
    {
        try
        {
            ...
            foreach (var row in grammarRules.Items)
            {
                ...
                var transitionString = $"{children[3].Text} {children[4].Text} {children[5].Text}";
    ...
    private void CheckClicked(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            for (var i = 0; i < processChain.Length; i++)
            {
                ...
                var currentChain = processChain[i..];
                translatedChain += sequence[^1] != EMPTY_SEQUENCE_SYMBOL ? sequence[^1] : "";

                var stackTop = stack.Pop();
                var stackSecond = stack.Count > 0 ? stack.Peek().ToString() : "";
                stack.Push(stackTop);

                var currentChainOutput = currentChain.Length > 1 ? currentChain[..^1] : EMPTY_SEQUENCE_GRAPH_SYMBOL;
                var translatedChainOutput = translatedChain.Length > 0 ? translatedChain : EMPTY_SEQUENCE_GRAPH_SYMBOL;

                Output += $"(" +
                    $"{_current_rule}, " +
                    $"{currentChainOutput}, " +
                    $"{stackTop}{stackSecond}, " +
                    $"{translatedChainOutput}" +
                    $") ├─ ";
    ...
    private void Button_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            var counter = 0;
            foreach (var line in File.ReadLines(@"D:\Projects\GithubProjects\SibSUTIS7semester\TYAP\lab6\DMPA.txt"))
            {
                if (counter == 0)
                {
                    Grammar = line;
